...
- **Phase 4 — Namespaces at scale** ✅ playlist/channel subscriptions polled on a schedule, the watch inbox as the signed-in home (Read / Chat / Skip), novelty triage from the 6th item, namespace summaries every 3 ingests, namespace-level chat with the retrieval tools.
...
**Chrome extension** — `apps/extension` (load unpacked): add the YouTube video / X post / LinkedIn post you are on to a namespace, and keep the server's YouTube session alive by sending a dedicated profile's cookies to `POST /youtube/cookies` (automatically when `/health` reports it stale). See `apps/extension/README.md`.
