...
export async function listExpressions(deps: { db: Db; storage: Storage }, itemId: string): Promise<{ item_id: string; title: string; expressions: ExpressionView[] } | null> {
  const [item] = await deps.db.select().from(items).where(eq(items.id, itemId));
  if (!item) return null;
  const doc = await getDocument(deps.storage, itemId);
  if (!doc) return null;
  const saved = await deps.db.select().from(expressionReviews).where(eq(expressionReviews.itemId, itemId));
...
/** "Learn": put the expression in the review queue (first recall prompt in 2 days). Idempotent. */
export async function saveExpression(deps: { db: Db; storage: Storage }, itemId: string, n: number): Promise<ExpressionReview> {
  const doc = await getDocument(deps.storage, itemId);
  const e = doc?.language_pack?.expressions[n];
  if (!doc || !e) throw new Error("expression not found");
  const [existing] = await deps.db.select().from(expressionReviews).where(and(eq(expressionReviews.itemId, itemId), eq(expressionReviews.n, n)));
...
export async function unsaveExpression(db: Db, itemId: string, n: number): Promise<boolean> {
  const rows = await db.delete(expressionReviews).where(and(eq(expressionReviews.itemId, itemId), eq(expressionReviews.n, n))).returning({ id: expressionReviews.id });
...
export async function reviewQueue(db: Db, opts: { now?: Date; limit?: number } = {}): Promise<{ due: ReviewCard[]; upcoming: ReviewCard[]; total: number }> {
...
export async function reviewSummary(db: Db, now = new Date()): Promise<{ due: number; total: number; next_due_at: string | null }> {
  const rows = await db.select({ dueAt: expressionReviews.dueAt }).from(expressionReviews).orderBy(asc(expressionReviews.dueAt));
...
export async function answerReview(db: Db, id: string, result: "got_it" | "again", now = new Date()): Promise<ExpressionReview | null> {
...
export async function dueReviews(db: Db, now = new Date()): Promise<ReviewCard[]> {
  return (await reviewQueue(db, { now })).due;
}
