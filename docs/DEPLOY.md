...
1. **Use a private window.** Open an incognito/private window, sign in to YouTube with the spare account, install *Get cookies.txt LOCALLY* (allow it in incognito), open youtube.com, export → `youtube-cookies.txt` (Netscape format) — then **close that window without signing out** and never open YouTube with that account in a normal window again. This is yt-dlp's own guidance: any browser tab using the account rotates the session, and the exported file dies with it. (That is what happened on the first day: the export worked once, then every request failed with "not a bot" — yt-dlp's verdict, visible on `/health` as `"youtube":"cookies_stale"` and on the card as *The YouTube cookies file is no longer valid*.)
2. Copy it to the box (the folder is git-ignored and mounted read-only into the container; yt-dlp works on a private copy of the file for every run and never rewrites yours):
   ```bash
...
**The jar is shared truth — let the server rewrite it.** YouTube rotates the session cookies on every authenticated request, yt-dlp's included, and whoever holds the rotated set owns the session. So the server gives each yt-dlp run a private copy (two runs never write the same file), writes the rotated jar back after a successful run, and the cookie keeper adopts a jar it did not write itself before its own hourly visit. (An earlier version threw yt-dlp's rotations away to keep the file pristine; that killed the session within the hour.) Never run yt-dlp by hand against the shared file — copy it first — and never open the account in a browser. **The check is about rate, not any one request.** Measured on the box: single, spaced requests with the cookies pass; a handful inside a minute (two workers, quick retries, someone pressing Retry repeatedly) and YouTube answers "not a bot" to everything for the next five to ten minutes — and every further request extends that. So the server sends one yt-dlp request at a time, 15 s apart, retries a bot check exactly once after four minutes, and the broker retries the job after 5 and then 10 minutes. If a card says *YouTube rejected this server's session just now*, leave it alone for ten minutes before pressing Retry; pressing it every minute keeps the address flagged. For real volume, the honest answer is a residential proxy (`YTDLP_PROXY`) or fetching from a non-cloud address. The production stack also runs a **PO-token provider** (`pot-provider` in `docker-compose.prod.yml`, the bgutil server; the yt-dlp plugin is in the server image and `YTDLP_POT_PROVIDER_URL` points at it), which makes the requests look legitimate to YouTube. `docker compose -f docker-compose.prod.yml exec server yt-dlp -v --simulate <url> 2>&1 | grep -i pot` shows the plugin talking to it. `curl https://api…/health` carries `"youtube": ok | cookies_stale | blocked | error` from a tiny probe at boot and every six hours; when in doubt, read that before pressing Retry. If a card still says *YouTube rejected this server's session*, press Retry; if it keeps happening for hours, the cookies have gone stale — export again.

**The cookie keeper (recommended on a cloud box).** Exported sessions die as soon as the account is used anywhere else, so the production stack runs a **cookie keeper** (`cookie-keeper` in `docker-compose.prod.yml`, `apps/keeper`): a headless Chromium with its own persistent profile that visits youtube.com every hour — Google rotates the cookies inside that browser — and rewrites `secrets/youtube-cookies.txt` for yt-dlp. It never signs in by itself; you seed it once:

1. Private window, spare Google account, export → `youtube-cookies.txt` (step 1 above), close the window, never open YouTube with that account again.
2. `scp` it to `~/marrow/secrets/youtube-cookies.txt` on the box. Within the hour (or `docker compose -f docker-compose.prod.yml restart cookie-keeper` to do it now) the keeper imports it — a jar it did not write itself is treated as a seed — and takes over.
3. `curl https://api…/health` → `"youtube_session": {"status":"ok","cookies":31,"checked_at":…}`. `needs_seed` = no session yet; `signed_out` = the session was revoked (re-export); `error` = the browser could not run (see `docker compose logs cookie-keeper`).

Memory: about 400 MB while a tick runs (a few seconds an hour), capped at 900 MB. Re-seeding is the same as seeding. Caveats worth saying out loud: it is the same grey zone as yt-dlp itself, and Google may eventually challenge an account that only ever appears from a datacenter — use a throwaway account, never your main one.
