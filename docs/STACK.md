...
| `STACK:diarization` | **Deferred** — `speakers: [S1]` fallback (PRD §15 Q3) | OpenAI-only stack: `gpt-4o-transcribe-diarize` gives speakers but no word timestamps, so it would be a second full STT pass. Stage exists, currently a no-op that records the fallback. |
...
| Web framework (Phase 3) | **Next.js (latest, App Router)** + React 19 + Tailwind v4 + shadcn/ui; Vercel AI SDK for chat streaming. |
...
| Hosting | **One EC2 instance** (`t4g.medium`, ARM, ~ $25/mo, Elastic IP) running **docker-compose**: `server` (bun + ffmpeg + yt-dlp image), `web` (Phase 3), `caddy` (automatic HTTPS). Subdomain = one A record at the registrar → Elastic IP; no Route 53, ALB, NAT Gateway, ECS, or CloudFront. Deliberately **not** Lambda (15-min cap, no ffmpeg) or Fargate (more moving parts than a first-time AWS user needs). Cloudflare Workers were evaluated and rejected (no native binaries). Known risk on any cloud: YouTube rate-limits datacenter IPs for yt-dlp — the CLI can ingest from the owner's Mac into the same S3/RDS. AWS setup guide to be generated at deployment time (Phase 3). Estimated total ≈ $40/mo, all credit-eligible. |
