...
| Hosting | **Web app on Vercel** (Next.js, root dir `apps/web`, env `MARROW_API_URL`/`MARROW_API_KEY`; the browser never talks to the API directly — the app's `/api/marrow/*` proxy does). **Server on AWS**: one EC2 instance (`t4g.small` + 2 GB swap, Elastic IP; the image bundles ffmpeg, yt-dlp and Deno for yt-dlp's YouTube challenge solver; an S3-only IAM role) running docker-compose (`server` + `caddy`) on `api.<domain>`; RDS PostgreSQL (pgvector) and one S3 bucket. No ALB/NAT/ECS/CloudFront/Route 53. Setup guide: `docs/DEPLOY.md`. |
...
| Accounts & RBAC | **Better Auth 1.7** inside the Hono server on the same Postgres (Drizzle adapter, tables `auth_*`), email + password with open sign-up; **`organization` plugin** = workspaces, members, invitation links, roles from `createAccessControl` (viewer / member / admin / owner over `namespace`, `item`, `source`, `chat`, `practice`, `apikey`); **`@better-auth/api-key`** = personal `mrw_…` keys bound to a workspace (MCP/Claude Code). The Next.js app proxies `/api/auth/*`, gates pages in `proxy.ts` + the `(app)` layout and forwards the cookie through its `/api/marrow` proxy. `MARROW_API_KEY` remains the instance key (`x-marrow-org` picks the workspace). |
| Client state (web) | **TanStack Query 5** for server state in client components (`lib/queries.ts`: me, workspace roster, API keys, practice, mutations) and **Zustand 5** with `persist` for per-browser UI preferences (`lib/store.ts`). Server components keep fetching through `lib/api.ts`. |
