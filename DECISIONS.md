...
## 2026-08-28 — YouTube's bot check is rate-driven; the queue must never burst

Watching one item on the box through nine failed attempts settled it: single, spaced yt-dlp requests with the same cookies pass every time; a handful inside a minute — two workers, in-stage retries, re-enqueued duplicates, an operator pressing Retry — and the address is flagged for five to ten minutes, and every further request extends it. Changes: an app-level guard (`shouldEnqueue`) so a running job is never sent again and the queue is created `stately`; at boot the broker's leftover waiting/active/retrying jobs are all cancelled and re-sent from the jobs table's truth, once each; one yt-dlp call at a time per process, 15 s apart; a bot check retried once after four minutes, the broker after five and then ten. Documented for the operator: wait ten minutes before Retry; use a proxy for volume. (PRD §5 fetch)

## 2026-08-28 — Stale cookies were the day's real culprit; the probes that said otherwise were wrong

Correction to the two entries above. Run on the owner's home connection with the exported file, yt-dlp itself said: *the provided YouTube account cookies are no longer valid — they have likely been rotated in the browser*. The account had kept being used in a normal browser window after the export, so the session rotated and the file died; from the AWS address that shows as "not a bot" on every request, while a home address passes even without cookies. Several of the day's "single spaced requests pass" readings came from a probe harness that printed OK whenever `head` exited 0 — i.e. also when yt-dlp printed nothing — so the rate-based theory rested partly on bad data; only the 06:44 ingest and one 13:47 probe were genuine passes. Kept: the no-duplicate guard, the gate, the sparse retries (all correct hygiene). Added: yt-dlp's stale-cookie verdict surfaces on the card and on `/health` (`youtube`, probed at boot and every six hours); `--no-warnings` dropped from metadata calls so the verdict is visible; one attempt without cookies before the long wait (an unflagged address needs none, and stale cookies are worse than none); DEPLOY now says to export from a private window and close it. (PRD §5 fetch)

## 2026-08-28 — The cookie keeper: a headless browser owns the YouTube session

Owner: "can we automate chrome … and download videos from yt through chrome or some headless browser" → build it. What a browser on the box cannot change: the address verdict (signed-out, a datacenter IP gets the same wall in Chrome as in yt-dlp; stream URLs are bound to the requesting IP anyway). What it does change: cookie rot — the exported session dies when the account is used anywhere else, so a headless Chromium that is the *only* user of a spare account keeps it alive indefinitely. Design: `apps/keeper`, a Bun + Playwright loop; a persistent profile; one short headless session an hour (launch → youtube.com → check `ytcfg.LOGGED_IN` → export YouTube/Google cookies as a Netscape jar → close, so memory is only used during a tick); seeded once from a private-window export dropped at the jar path (a jar the keeper did not write is a seed) — no login is ever automated; yt-dlp keeps taking private copies of the jar; the keeper's report shows on `/health.youtube_session`. yt-dlp's own cookie write-back would do a weaker version of the same, but a real browser handles Google's rotation and consent flows, and the status is explicit. Caveats recorded in DEPLOY: same grey zone as yt-dlp; throwaway account only. (PRD §5 fetch; owner request)

## 2026-08-28 — Transcription chunks are capped by duration, not just bytes

The first long ingest through the keeper's session failed at transcription with "Request timed out": the planner only split audio at the 25 MB cap, so 2 h 38 m of low-bitrate audio became two ~80-minute chunks, and one of them outlived the SDK's request timeout. Chunks are now also capped at `STT_CHUNK_MAX_S` (10 min; cut at silences, three in flight), which is both faster — pieces transcribe in about a minute each in parallel — and finer-grained for retries, and each request carries an explicit `STT_REQUEST_TIMEOUT_MS` (15 min). Word timestamps are offset per chunk as before. (PRD §5 transcribe, §4.3 word timestamps)

## 2026-08-28 — Rotated cookies must be written back; the jar is shared truth

The keeper's first real day: seeded at 14:52, the box fetched a 2.6-hour video at 14:53 with a private copy of the jar, and by 15:12 both yt-dlp and the keeper's browser reported the session gone. YouTube rotates the session cookies on every authenticated request, including yt-dlp's; the private-copy design (made to stop yt-dlp "shrinking" the export, which was never the real problem) discarded exactly the rotation that keeps the session alive. Now: each run still works on a private copy, but a successful run writes the rotated jar back atomically; the probe does the same; the keeper imports any jar it did not write before its hourly visit; the server mounts `/secrets` writable again. One rule for the operator: the account exists for the server alone. (PRD §5 fetch)

## 2026-08-28 — Items can move between namespaces

Owner: a video was retried into the wrong namespace and there was no way to move it; "language namespaces should be able to work also". Moving is now a first-class operation (REST, MCP, a select in the item's meta line for anyone who may add items). Everything namespace-scoped follows the item — search segments, ledger rows, entity mentions (rebuilt in the target from the document without an LLM call; the source's orphaned entities are pruned) — and what a namespace *decides* is recomputed by a small job at the same document version that forgets only the `language` and `novelty` stage records: novelty against the new corpus, and expressions when the target is in language mode. The same link already finished in the target refuses the move (open that one); a stalled copy gives way. (PRD §6.4 namespaces, §6.3 language mode, §9 entity index)

## 2026-08-28 — The landing page: one hero, an ASCII brain, no image sections

Owner-directed, over a long review loop on the dev server. Positioning: "a research brain, grown from what you consume" — podcasts, YouTube, posts, newsletters, papers in, research out. Tried and rejected on the way: Roman paintings/etchings as full-bleed bands (first "no image cards", then "keep the UI clean"), plates from researchers' notebooks, dot-matrix classical art in the inspiration style, a procedural 3D brain and a CC BY-SA brain mesh (both "not identical"). What stayed: the owner's own brain image (a grey wireframe render; the blue one is kept in `public/landing/`) drawn as live ASCII on a canvas with the image's own colour, slow motion and firing neurons; a floating blurred header with `rounded-2xl` corners and only *Sign in*; a single CTA *Grow your research brain*; a plain mono *Built by Saidev Dhal* link; no other sections, no footer, no dotted-grid background. The front door is `/` — `/welcome` is only the rewrite target and redirects when typed — and a stale session cookie shows the landing (one `GET /me` from the proxy on `/`) instead of a sign-in bounce. OpenGraph images share the brain as text. (PRD §6.4 — the inbox stays the signed-in home; §11 web app)

## 2026-08-28 — YouTube cookies come from a Chrome extension, which also captures

The recurring outage was never yt-dlp: it was the manual export of a signed-in cookie jar and the scp to `/secrets` every time the session died. `POST /youtube/cookies` (owner or instance key) now accepts a Netscape jar, validates that it is a signed-in YouTube session, writes it atomically to `YTDLP_COOKIES` and re-probes; the keeper treats it as a seed. The owner's browser sends it via a private MV3 extension (`apps/extension`) — on demand, or hourly but only when `/health` reports a stale session, so a healthy server-rotated jar is never clobbered. The same popup adds the YouTube video / X post / LinkedIn post the owner is looking at to a namespace; social posts are captured from the page open in the owner's own browser (text + author) and sent to `POST /capture`, which keeps the "no server-side scraping, no login automation" line intact. (PRD §3 constraints, §7 capture)

## 2026-08-29 — The extension: one pasted key, in-page buttons, cookies invisible

Owner: "users don't know the API address … just ask the user to generate an API key from our dashboard", "don't show users the send-cookies option", and "there should be an icon in the YT video player itself … same for posts, I don't want to click the extension again and again". So: the API and web addresses are constants in `apps/extension/config.js` (overridable under Settings → Advanced); setup is one pasted key validated against `GET /me`; a *Save to Marrow* control is injected into the YouTube player's control bar and into X/LinkedIn post action rows (left-click saves to the last namespace, right-click picks one, a shadow-root toast confirms); the cookie courier has no visible affordance — it runs hourly-when-stale and before a YouTube add while stale, owners' keys only, with a quiet "Send YouTube session now" under Advanced. Verified on a live YouTube page: YouTube enforces Trusted Types, so the content script builds DOM nodes and never touches `innerHTML`. (PRD §3 constraints, §7 capture)
