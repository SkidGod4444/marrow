...
apps/web/              Next.js (App Router): app/page.tsx (inbox = landing), app/library, app/items/[id] (item, ?t=&tab=), app/namespaces/[name]/{chat,graph},
...
**Reading/export surfaces.** `services/export.ts` renders markdown (`documentToMarkdown`, dialogue-aware) and plain text (`documentToText`); routes `/items/:id/export.md|txt`; web `/items/[id]/read` + `ReadToolbar` (copy/download/print/share). Dates are formatted with `fmtDate` (UTC ISO) in client components — never `toLocale*` — to avoid hydration mismatches; the YouTube player is only created for well-formed 11-char ids.
