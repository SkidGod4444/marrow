...
const PUBLIC = [/^\/login$/, /^\/signup$/, /^\/items\/[^/]+\/read$/, /^\/api\/marrow\/public\//, /^\/sitemap\.xml$/, /^\/robots\.txt$/, /^\/api\/auth\//, /^\/api\/version$/, /^\/_next\//, /^\/_vercel\//, /^\/favicon\.ico$/, /^\/icon/, /^\/apple-icon/, /^\/opengraph-image/, /^\/brand\//, /^\/robots\.txt$/, /^\/manifest/];
