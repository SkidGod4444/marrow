...
export function ReadView({ doc }: { doc: PresentedDocument }) {
  ...
  return (
    <PlayerProvider videoId={videoId}>
      <article className="mx-auto max-w-3xl space-y-8">
        ...
        <header className="space-y-3">
          <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{doc.channel || "Shared from Marrow"}</p>
        ...
        {/* The full transcript as speaker-labelled dialogue, on request. */}
        <section className="space-y-4 border-t border-border/70 pt-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
              Transcript{entries.length ? ` · ${entries.length} lines` : ""}
              {multi ? ` · ${speakers.length} speakers` : ""}
            </p>
            {entries.length > 0 && (
