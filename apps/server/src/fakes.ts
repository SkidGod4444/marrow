...
export async function seedFakeCorpus(deps: { db: Db; storage: Storage; config: Config; providers: ReturnType<typeof fakeProviders> }, log: (m: string) => void): Promise<void> {
  const { db, storage, config, providers } = deps;
  if ((await listNamespaces(db)).length) {
    log("fake corpus already present");
    return;
  }
  const ns = (await getNamespace(db, "demo")) ?? (await createNamespace(db, { name: "demo", description: "Sim-to-real robot learning" }));
  ...
  const failed = await createIngest(db, { namespace: ns.name, url: "https://www.youtube.com/watch?v=broken-download" });
